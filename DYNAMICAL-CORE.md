...
The `SimUnits` class (`units.py`) stores non-dimensional physical constants (g, R, κ, R_vapor, Cp, Cp_vapor) and provides `nondimensionalize()` / `dimensionalize()` methods.
...
### Strengths
- **Complete and correct**: Implements the standard spectral primitive equations with all the classical numerical techniques (semi-implicit IMEX, dealiasing, spectral filtering)
- **Fully differentiable by design**: Written from scratch in JAX specifically for end-to-end gradient-based learning
- **Multiple vertical coordinates**: Both pure sigma and hybrid sigma-pressure, with ECMWF L137 levels available
- **Parallelism-ready**: SPMD sharding for multi-device execution
- **Well-referenced**: Code comments cite Durran (2010), Simmons & Burridge (1981), Whitaker & Kar (2013) for numerical methods

### Limitations (deliberate design choices for the NeuralGCM context)
- **Hydrostatic only**: No non-hydrostatic extension. This limits applicability to horizontal resolutions coarser than ~10 km
- **No semi-Lagrangian advection**: Uses Eulerian advection only (centered differences or semi-Lagrangian in the vertical only). This limits the maximum stable timestep compared to semi-Lagrangian schemes
- **No built-in physics**: Radiation, convection, boundary layer, precipitation are all delegated to NeuralGCM's neural parameterizations
- **Spectral limitations with topography**: Gibbs phenomenon near sharp orography, partially mitigated by filtering and learnable corrections
- **float32 transforms**: The fast spherical harmonic transforms default to `tensorfloat32`, which may introduce small numerical errors in long integrations
